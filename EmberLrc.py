...
def UpdateCover(file):
    base_name = os.path.basename(file)
    file_name = os.path.splitext(base_name)[0]
    abs_path = os.path.abspath(os.path.dirname(file)) + os.path.sep
    ...
    img_name = abs_path + 'cover\\' +singer + '.jpg'
    image = None
    ...
    if subffix == '.mp3':
        ...
        if audio.getall('APIC'):
            ...
            if os.path.isfile(img_name):
                ...
                print('Can\'t find cover file for {0} ...'.format(file))
                return 0

            audio['APIC'] = APIC(encoding = 3, data = image)
            audio.save()

            print('update {0} cover success'.format(file_name))
            return 2
    elif subffix == '.flac':
        audio = FLAC(file)
...
for audioName in audioList.keys():
    if ' - ' in audioName:
        ret = UpdateCover(audioList[audioName])
        if ret == 0:
            failure += 1
        elif ret == 2:
            success += 1
